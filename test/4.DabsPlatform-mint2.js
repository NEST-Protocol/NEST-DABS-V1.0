...
describe('4.DabsPlatform-mint2', function() {
    it('First', async function() {
        ...
        const estimateMint = async function(amount) {
            let prices = await nestBatchPriceView.lastPriceList(0, 0, 2);
        ...
        if (true) {
            console.log('4. swap');
            await hbtc.approve(cofixRouter.address, toBigInt(1000));
            await skipBlocks(0);
            await cofixRouter.swapExactTokensForTokens(
                [hbtc.address, dabsStableCoin.address],
                toBigInt(100),
                0n,
                owner.address,
                owner.address,
            ...
            console.log('swap-v: ' + v);

            aeq(v, status.owner.stable);

            prev = parseFloat(status.owner.hbtc);
        }
        ...
        if (false) {
            ...
            let b = parseFloat(status.owner.hbtc) - prev;
            console.log('burn-v: ' + v);
            aeq(v, b);
        }

        let dabsStableCoinAmount = await dabsStableCoin.balanceOf(owner.address);
        if (true) {
            console.log('6. stake');
