...
exports.calcRevisedK = function(sigmaSQ, p0, bn0, p, bn, currentBlock) {
    sigmaSQ = parseFloat(sigmaSQ);
    p0 = parseFloat(p0);
    bn0 = parseFloat(bn0);
    p = parseFloat(p);
    bn = parseFloat(bn);
    ...
    sigmaISQ = sigmaISQ * sigmaISQ / (bn - bn0) / BLOCK_TIME;

    if (sigmaISQ > sigmaSQ) {
        k += Math.sqrt(BLOCK_TIME * sigmaISQ * (currentBlock - bn));
    } else {
        k += Math.sqrt(BLOCK_TIME * sigmaSQ * (currentBlock - bn));
    }
